...
struct InputRow
{
    ...
    friend std::ostream &operator<< (std::ostream &out, InputRow &row)
    {
        ...
        out << ",";
        if (row.year != -1 && row.year != -2) out << row.year;
        out << ",";
        if (row.crop_land != -1 && row.crop_land != -2) out << row.crop_land;
        out << ",";
        if (row.grazing_land != -1 && row.grazing_land != -2) out << row.grazing_land;
        out << ",";
        if (row.forest_land != -1 && row.forest_land != -2) out << row.forest_land;
        out << ",";
        if (row.fishing_ground != -1 && row.fishing_ground != -2) out << row.fishing_ground;
        out << ",";
        if (row.built_up_land != -1 && row.built_up_land != -2) out << row.built_up_land;
        out << ",";
        if (row.carbon != -1 && row.carbon != -2) out << row.carbon;
        out << ",";
        if (row.total != -1 && row.total != -2) out << row.total;
        out << ",";
        if (row.percapita != -1 && row.percapita != -2) out << row.percapita;
        out << ",";
        if (row.population != -1 && row.population != -2) out << row.population;
